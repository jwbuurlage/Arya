...
namespace Arya
{
	...
	bool Camera::updateViewMatrix(mat4* outMatrix){
		if( updateMatrix ){
            *outMatrix = glm::translate( mat4(1.0f), -position );
            *outMatrix = glm::rotate( *outMatrix, -yaw, vec3(0.0, 1.0, 0.0) );
            *outMatrix = glm::rotate( *outMatrix, -pitch, vec3(1.0, 0.0, 0.0) );
            *outMatrix = glm::translate( *outMatrix, vec3(0, 0, -camDist) );
