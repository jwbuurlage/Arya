...
//MODIFIED BY TOMBANA
//Added logaritmic capacity scaling when appending
...
inline void buffer::assign (const void* d, size_type s)
{
  if (s > capacity_)
  {
    ...
    data_ = new char[2*s]; //changed by Tom
    capacity_ = 2*s; //changed by Tom
    free_ = true;
  }
...
inline bool buffer::resize (size_type s)
{
  bool r (false);
